	...
	*/
	public function search_result($queryTerm,$origin = 1)
	{
		...
		if($queryTerm=='')
		{
			$tagsArray= array();
			                    $allfilters = array();
								$allfilters = $this->search_model->getAllfilters_new();
								...
								$Fees = $allfilters[5];
			                     ...
			                     $fee =$this->search_model->getfield($rank,10);
	...
	/*	foreach($data['discussion']['questions'] as $row)
	    ...
	    if($getquery->num_rows())
		{
								...
								$sortby = intval($queryrow[0]->sortby);
								//code for auto filters check
								
								//according to new table2
								$filter=explode(',',$filter);
								$str = array(); $attr = array(); $psy = array();
								$j=0;
								$node_type = array(); $node_id = array(); $equality = array(); $node_value = array();
								$stream = array(); $major = array(); $degree = array(); $country = array();

								foreach($filter as $sub_filter)
								{
									$sub_filter = explode('|',$sub_filter);
									$i=0;
									
									foreach($sub_filter as $val)
									{
										if($i%4==0)
										{
											$node_type[] = $val;
											if($val=='S')
											{
												if($sub_filter[2]=='1')
												{
													array_push($stream,$sub_filter[1]);
												}
												else if($sub_filter[2]=='2')
												{
													array_push($degree,$sub_filter[3]);
												}
												else if($sub_filter[2]=='3')
												{
													array_push($major,$sub_filter[1]);
												}
											}
										}
										else if($i%4==1)
								...
								//end of according to new table2
								/*
								...
								*/
								...
								$collegelistid = $this->search_model->filterslist_new($node_type,$node_id,$equality,$node_value);
								//echo '<pre>'; print_r($collegelistid); echo '</pre>';
								$rank=$this->search_model->filters_ranking(array(),array_values($collegelistid),$sortby,''); //sortby should be passed instead of '1'
								//echo '<pre>'; print_r($rank); echo '</pre>';
								//$rank[0]['header'] = $_SESSION['field'];
								$_SESSION['rank'] = $rank;
								...
								$fee =$this->search_model->getfield($rank,10);
			                     ...
			                     //echo '<pre>'; print_r($stream); echo '</pre>';
			                     //echo '<pre>'; print_r($data); echo '</pre>';			                     
			                     //print_r($data['mark_stream'][0]['SNode_Name']);
			$this->load->view('search_result',$data); 
		}
		 else 
		 {
	...
	{   
		...
		//degree clicked
			...
			for($i=0;$i<sizeof($Streams1);$i++)
			{
				...
				{
					if($stream_major_nest[$j]==$Streams1[$i]["Snode_Name"])
					{
						$Streams1[$i]['value'] = 1;
						break;
					}
			...
			for($i=0;$i<sizeof($Degrees1);$i++)
			{
				...
				{
					if($stream_nest[0][$j]==$Degrees1[$i]["Snode_Name"])
					{
						$Degrees1[$i]['value'] = 1;
						break;
					}
			...
			for($i=0;$i<sizeof($Majors1);$i++)
			{
				...
				{
					if($stream_nest[2][$j]==$Majors1[$i]["Snode_ID"])
					{
						$Majors1[$i]['value'] = 1;
						break;
					}
		...
		//filter nested loop ends
			...
			if((sizeof($collegelistid)==0)&&((sizeof($stream)+sizeof($major)+sizeof($degree)+sizeof($country))>0))
			{
				...
				if(sizeof($collegelistid))
				{
					...
					{
						$rank2=$this->search_model->filters_ranking($tags,$collegelistid,$sorting,$majorTag);
						$rank = array_intersect($_SESSION['rank'], $rank2);
					}
					else
					{
