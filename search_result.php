					...
					<div id="leftPanel" class="lbar">
						...
						<iron-pages selected="{{selected}}">
							<div class="left-panel-pages" style="margin-bottom: 10px;">
								...
								<span>
									<div id="filter-res" style="margin-bottom: 5px;">
										<div id="found"> Found <?php if( sizeof($rank)>=10){echo '10';} 
										else{
												echo sizeof($rank);
										} ?> Colleges</div>
								...
								<span>
									...
									<div class="filterName" aria-expanded$="[[isExpanded()]]" aria-controls="State-div" onclick="toggle('#State-div')">
										...
										<div id = "name-detail"><?php echo strtoupper($Parents[1]['Snode_Type']); ?></div><!-- <div id="cross"><iron-icon icon="clear"></iron-icon></div>  -->
									</div>
								</span>
								...
								<div class="listOptions" id='stream'>
									...
									{
										echo '<div class="categoryOptions"><paper-checkbox noink onchange="filter()" ';
										for($j=0;$j<sizeof($mark_stream);$j++)
										{
											if($Streams[$i]['Snode_ID']==$mark_stream[$j])
												{ print "checked";} 
										}    
										echo'    >'.$Streams[$i]["Snode_Name"].'</paper-checkbox> <input type="hidden" id="streamid" value="'.$Streams[$i]["Snode_ID"].'" /></div>';
										$i++;
									}
							...
							<div class="filterName" aria-expanded$="[[isExpanded(opened3)]]" aria-controls="majors-div" onclick="toggle('#majors-div')">
								<div id = "name-detail"><?php echo strtoupper($Parents[2]['Snode_Type']); ?></div> <!-- <div id="cross"><iron-icon icon="clear"></iron-icon></div> -->
							</div>
							<iron-collapse id="majors-div" >
								...
								<div class="listOptions" id='degree'>
									...
									{
										echo '<div class="categoryOptions"><paper-checkbox  onchange="filter()" ';for($j=0;$j<sizeof($mark_degree);$j++){if($Degrees[$i]['Snode_Name']==$mark_degree[$j]){ print "checked";}}  echo'>'.$Degrees[$i]["Snode_Name"].'</paper-checkbox></div>';
										$i++;
									}
							...
							<div class="filterName" aria-expanded$="[[isExpanded(opened3)]]" aria-controls="majors-div2" onclick="toggle('#majors-div2')">
								<div id = "name-detail"><?php echo strtoupper($Parents[3]['Snode_Type']); ?></div> <!-- <div id="cross"><iron-icon icon="clear"></iron-icon></div> -->
							</div>
							<iron-collapse id="majors-div2" >
								...
								<div class="listOptions" id="major" >
									...
									{
										echo '<div class="categoryOptions" ><paper-checkbox  onchange="filter()" ';for($j=0;$j<sizeof($mark_majors);$j++){if($Majors[$i]['Snode_ID']==$mark_majors[$j]){ print "checked";} }  echo'>'.$Majors[$i]["Snode_Name"].'</paper-checkbox> <input type="hidden" id="majorid" value="'.$Majors[$i]["Snode_ID"].'" /></div>';
										$i++;
									}
...
<div id = 'college_list'>
	...
	for ($i=0;$i<$show_length;$i++)
	{
		...
		{
			$heading = $_SESSION['field_name'];
			$heading2 = $_SESSION['field_name2'];
			$value ='<b>'.$rank[$i]['field'].'</b>';
			$value2 ='<b>'.$rank[$i]['field2'].'</b>';
		}
		echo	'<paper-card>';
		...
		echo			'<div class="det1 fnew"><span class="newfnt">'.$heading2.':</span><b>'.$value2.'</b></div><br>';
						...
						{ 
							...
							for(var i=0;i<data["rank"].length;i++)
							{
								...
								if(data["tag_name"] == 1)
								{
									...
									var heading2 = "<?php echo $_SESSION['field_name']?>";
									
									if(data["rank"][i]["field"] == "INT_MAX" || data["rank"][i]["field"] == -1 )
									{
										var value2 = "NA";
									}
									else
									{
										var value2 = data["rank"][i]["field"];
								...
								else 
								{
									...
									if(sorting==0){
										heading = "<?php echo $_SESSION['field_name']?>";
										heading2 = 'Annual Expenses';
									}							
									else if(sorting == 1){
										heading = 'Annual Expenses';
										heading2 = 'Starting Annual Salary';
									}
									...
									{
										heading = 'Starting Annual Salary';
										heading2 = 'Annual Expenses';
									}

									else if(sorting == 6 || sorting == 7)
									{
										heading = 'Class Strength';
										heading2 = 'Annual Expenses';
									}
									...
									{
										var value = data["rank"][i]["field"];
										
									}
									if(data["rank"][i]["field2"] == "INT_MAX" || data["rank"][i]["field2"] == -1 )
									{
										var value2 = "NA";
									}
									else
									{
										var value2 = data["rank"][i]["field2"];
									}
								...
								$('<paper-card> <div class="college"> <a href="'+site_url+'college/details/'+data["rank"][i]["encoded_id"]+'" style="cursor: pointer;">'+
									...
									'<div class="other-details"> <div class="detail1"><div class="det1"><span style="font-weight: 600;">Streams: </span>'+data["rank"][i]["streams"]+'</div><br> <div class="det1-res" style="margin-top: 7px;">Degrees: <span style="font-weight: 400; ">'+ data["rank"][i]["degree"]+'</span></div> </div></div></div></a><div class="right-up flex-2"><div class="detail1"><div class="det1 fnew"><span class="newfnt">'+heading+':</span><b>'+ value +'</b></div><br><div class="det1 fnew"><span class="newfnt">'+heading2+':</span><b>'+ value2 +'</b></div><br><div class="det1 fnew"><span class="newfnt">Profile Views:</span>'+data["rank"][i]["views"]+'</div></div><a href="' +site_url+ 'college/add_compare/'+data["rank"][i]["encoded_id"]+'" style="cursor: pointer;"> <div class="compare-button"> <paper-button> ADD TO COMPARE</paper-button> </div> </a> </div> </div></div> </paper-card>').appendTo('#college_list');
...
$('#found').html('Found '+data['rank'].length+' Colleges');
	...
	if(data['Streams'])
	{
		...
		for(var i=0;i<data['Streams'].length;i++){
			str = data['Streams'][i]['Snode_Name'];
			//window.alert(5);
		//str = str.replace("Yes_", ""); 
		var Streams = '<div class="categoryOptions"><paper-checkbox noink onchange="filter()" >'+str+'</paper-checkbox><input type="hidden" id = "streamid" value="'+data['Streams'][i]['Snode_ID']+'" /> </div>';
		if(data["Streams"][i]["value"]==0)
		{
			Streams = '<div class="categoryOptions"><paper-checkbox noink disabled onchange="filter()" >'+str+'</paper-checkbox><input type="hidden" id = "streamid" value="'+data['Streams'][i]['Snode_ID']+'" /> </div>';
		}
		if(data['mark_stream'])
		{
			for(var j=0;j<data['mark_stream'].length;j++)
			{
				...
				if(data['mark_stream'][j]==data['Streams'][i]['Snode_ID'])
				{
					Streams = '<div class="categoryOptions"><paper-checkbox noink onchange="filter()" checked >'+str+'</paper-checkbox><input type="hidden" id = "streamid" value="'+data['Streams'][i]['Snode_ID']+'" /> </div>';
	...
	if(data['Majors'])
	{
		...
		for(var i=0;i<data['Majors'].length;i++){
			str = data['Majors'][i]['Snode_Name'];
		//str = str.replace("Yes_", ""); 
		var major = '<div class="categoryOptions"><paper-checkbox noink onchange="filter()" >'+str+'</paper-checkbox><input type="hidden" id = "majorid" value="'+data['Majors'][i]['Snode_ID']+'" /> </div>';
		if(data["Majors"][i]["value"]==0)
		{
			major = '<div class="categoryOptions"><paper-checkbox noink onchange="filter()" disabled >'+str+'</paper-checkbox><input type="hidden" id = "majorid" value="'+data['Majors'][i]['Snode_ID']+'" /> </div>';
		}
		...
		if(data['mark_majors'])
		{
			...
			{
				if(data['mark_majors'][j]==data['Majors'][i]['Snode_ID'])
				{
					major = '<div class="categoryOptions"><paper-checkbox noink onchange="filter()" checked >'+str+'</paper-checkbox><input type="hidden" id = "majorid" value="'+data['Majors'][i]['Snode_ID']+'" /> </div>';
...
function addProfiles(collegeOrder,collegeData,div)	//Not Called First Time
{
	for(i = 0;i<collegeOrder.length;i++)
	{
		...
		if(college["data"].hasOwnProperty('Streams'))
		{
			term = term + '<div class="college-details"><span>Streams Offered</span>';
			for(j = 0;j<collegeData[collegeOrder[i]]["data"]["Streams"].length;j++)
			{
				term = term + '<div class="college-light">'+collegeData[collegeOrder[i]]["data"]["Streams"][j]+'</div>';
			}
			term = term + '</div>';
		}
